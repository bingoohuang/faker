...
func TestSetRandomStringLength(t *testing.T) {
	...
	if err := Fake(&someStruct); err != nil {
...
func TestSetRandomMapAndSliceSize(t *testing.T) {
	someStruct := SomeStruct{}
	if err := SetRandomMapAndSliceSize(-1, 0); err == nil {
		t.Error("Random Map and Slice must not accept lower than 0 as a size")
	}
	size := 5
	if err := SetRandomMapAndSliceSize(0, size); err != nil {
...
func TestExtend(t *testing.T) {
	...
	t.Run("test-string", func(t *testing.T) {
		a := struct {
			...
			Bingoo string `faker:"bingoo,k=v"`
		}{}

		if err := AddProvider("test", func(v reflect.Value) (interface{}, error) {
			return "test", nil
		}); err != nil {
			t.Error("Expected Not Error, But Got: ", err)
		}
